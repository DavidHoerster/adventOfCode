...
type alias MinuteKey =
    Int


type alias MinuteCount =
    Int
...
type alias GuardLog =
    Dict GuardId GuardMinuteLog


type alias SleepyGuard =
    { guard : GuardId
    , totalMinutes : Int
    , sleepiestMinute : Int
    }
...
init =
    ...
    in
    ( List.length (Dict.keys log), Cmd.none )


findSleepyGuard : GuardLog -> List GuardId -> SleepyGuard -> SleepyGuard
findSleepyGuard log guards leader =
    case guards of
        nextGuard :: remainingGuards ->
            let
                glog =
                    Maybe.withDefault Dict.empty (Dict.get nextGuard log)

                sleepyg =
                    findSleepyGuardHelper nextGuard glog
            in
            if sleepyg.totalMinutes > leader.totalMinutes then
                findSleepyGuard log remainingGuards sleepyg

            else
                findSleepyGuard log remainingGuards leader

        [] ->
            leader


findSleepyGuardHelper : GuardId -> GuardMinuteLog -> SleepyGuard
findSleepyGuardHelper guard log =
    let
        totalMins =
            Dict.foldl addMinutes 0 log

        maxMin =
            Dict.foldl findMax ( -1, -1 ) log
    in
    { guard = guard, totalMinutes = totalMins, sleepiestMinute = Tuple.first maxMin }
...
buildGuardLog : List Activity -> GuardLog
buildGuardLog acts =
    case acts of
        x :: xs ->
            buildGuardLogHelper x { guard = "", start = 0, end = 0 } Dict.empty xs

        [] ->
            Dict.empty


buildGuardLogHelper : Activity -> GuardStatus -> GuardLog -> List Activity -> GuardLog
buildGuardLogHelper act stat currLog remainingActs =
    case act of
        Start g l ->
            let
                newlog =
                    if Dict.member g currLog then
                        currLog

                    else
                        Dict.singleton g Dict.empty
            in
            case remainingActs of
                x :: xs ->
                    buildGuardLogHelper x { guard = g, start = 0, end = 0 } newlog xs
        ...
        FallsAsleep l ->
            case remainingActs of
                x :: xs ->
                    buildGuardLogHelper x { stat | start = l.minute } currLog xs
        ...
        WakesUp l ->
            let
                ...
                guardlog =
                    Maybe.withDefault Dict.empty (Dict.get stat.guard currLog)

                updatedGuardMinuteLog =
                    updateSleepLog guardlog stat.start stat.end

                newCurrLog =
                    Dict.insert stat.guard updatedGuardMinuteLog currLog
            in
            case remainingActs of
                x :: xs ->
                    buildGuardLogHelper x { guard = stat.guard, start = 0, end = 0 } newCurrLog xs
...
updateSleepLog : GuardMinuteLog -> Int -> Int -> GuardMinuteLog
updateSleepLog log currMin maxMin =
    if currMin == maxMin then
        ...
        let
            newMinValue =
                Maybe.withDefault 0 (Dict.get currMin log) + 1

            newLog =
                Dict.insert currMin newMinValue log
        in
        updateSleepLog newLog (currMin + 1) maxMin


parseLog : List String -> List Activity
parseLog list =
    List.map parseLogHelper list


parseLogHelper : String -> Activity
parseLogHelper s =
    let
        strYear =
            String.slice 1 5 s
        ...
        strHr =
            String.slice 12 14 s

        strMin =
            String.slice 15 17 s

        strKey =
            String.slice 19 24 s

        min =
            Maybe.withDefault -1 (String.toInt strMin)

        guard =
            String.trim (String.slice 26 30 s)

        log =
            { year = strYear, month = strMon, day = strDay, hour = strHr, minute = min }
    in
    case strKey of
        "Guard" ->
            Start guard log
...
view model =
    div []
        ...
        , h1 [] [ text ("Answer is :: " ++ String.fromInt model) ]
        ]
