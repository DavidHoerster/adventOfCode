...
var author$project$Main$getTopNodes = F3(
	function (g, nodes, roots) {
		getTopNodes:
		while (true) {
			if (nodes.b) {
				var x = nodes.a;
				...
				var ctx = A2(
					...
					{incoming: elm_community$intdict$IntDict$empty, node: x, outgoing: elm_community$intdict$IntDict$empty},
					A2(elm_community$graph$Graph$get, x.id, g));
				if (elm_community$intdict$IntDict$isEmpty(ctx.incoming)) {
					var $temp$g = g,
						$temp$nodes = xs,
						$temp$roots = _Utils_ap(
						roots,
						_List_fromArray(
							[x.label]));
					g = $temp$g;
...
var author$project$Main$constructStepOrderHelper = F2(
	function (g, currOrder) {
		constructStepOrderHelper:
		while (true) {
			if (elm_community$graph$Graph$isEmpty(g)) {
				return currOrder;
			} else {
				var roots = A3(
					author$project$Main$getTopNodes,
					g,
					elm_community$graph$Graph$nodes(g),
					_List_Nil);
				var sortedRoots = elm$core$List$sort(roots);
				var labelToRemove = A2(
					elm$core$Maybe$withDefault,
					_Utils_chr('$'),
					elm$core$List$head(sortedRoots));
				var newOrder = _Utils_ap(
					currOrder,
					_List_fromArray(
						[labelToRemove]));
				var idToRemove = elm$core$Char$toCode(labelToRemove);
				var updatedGraph = A2(elm_community$graph$Graph$remove, idToRemove, g);
				var $temp$g = updatedGraph,
					$temp$currOrder = newOrder;
				g = $temp$g;
				currOrder = $temp$currOrder;
				continue constructStepOrderHelper;
			}
		}
	});
var author$project$Main$constructStepOrder = function (g) {
	return A2(author$project$Main$constructStepOrderHelper, g, _List_Nil);
};
var elm$core$String$slice = _String_slice;
var author$project$Main$parseStep = function (s) {
	return _Utils_Tuple2(
		A3(elm$core$String$slice, 5, 6, s),
		A3(elm$core$String$slice, -12, -11, s));
};
...
var author$project$Main$createStepGraphHelper = F2(
	function (step, _n0) {
		var nodes = _n0.a;
		var edges = _n0.b;
		var parsedStep = author$project$Main$parseStep(step);
		var nodeToName = A2(
			elm$core$Maybe$withDefault,
			_Utils_chr('$'),
			elm$core$List$head(
				elm$core$String$toList(parsedStep.b)));
		var nodeFromName = A2(
			elm$core$Maybe$withDefault,
			_Utils_chr('$'),
			elm$core$List$head(
				elm$core$String$toList(parsedStep.a)));
		var nodeFromNameCode = elm$core$Char$toCode(nodeFromName);
		var newToNameCode = elm$core$Char$toCode(nodeToName);
		var newNodeDict = A2(elm$core$Dict$member, nodeFromName, nodes) ? nodes : A3(
			elm$core$Dict$insert,
			nodeFromName,
			A2(elm_community$graph$Graph$Node, nodeFromNameCode, nodeFromName),
			nodes);
		var newNodeDict2 = A2(elm$core$Dict$member, nodeToName, newNodeDict) ? newNodeDict : A3(
			elm$core$Dict$insert,
			nodeToName,
			A2(elm_community$graph$Graph$Node, newToNameCode, nodeToName),
			newNodeDict);
		var newEdges = _Utils_ap(
			edges,
			_List_fromArray(
				[
					A3(elm_community$graph$Graph$Edge, nodeFromNameCode, newToNameCode, _Utils_Tuple0)
				]));
		return _Utils_Tuple2(newNodeDict2, newEdges);
	});
...
var author$project$Main$init = function () {
	var partGraph = author$project$Main$createStepGraph(
		elm$core$String$lines(author$project$Main$input));
	var stepOrder = author$project$Main$constructStepOrder(partGraph);
	return _Utils_Tuple2(
		elm$core$String$fromList(stepOrder),
