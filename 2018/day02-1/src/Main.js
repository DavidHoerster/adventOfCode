...
var author$project$Main$checkWordStatsHelper = F2(
	function (word, count) {
		checkWordStatsHelper:
		while (true) {
			if (word.b) {
				var x = word.a;
				var xs = word.b;
				var newCount = function () {
					if (A2(elm$core$Dict$member, x, count)) {
						var currCount = function () {
							var _n1 = A2(elm$core$Dict$get, x, count);
							if (_n1.$ === 'Just') {
								var a = _n1.a;
								return a;
							} else {
						...
						return A3(elm$core$Dict$insert, x, currCount + 1, count);
					} else {
						return A3(elm$core$Dict$insert, x, 1, count);
					}
				}();
				var $temp$word = xs,
					$temp$count = newCount;
...
var author$project$Main$hasACount = F3(
	function (n, k, v) {
		return _Utils_eq(n, v);
	});
...
var author$project$Main$checkWordStatus = function (word) {
	var counts = A2(author$project$Main$checkWordStatsHelper, word, elm$core$Dict$empty);
	var hasThrees = A2(
		elm$core$Dict$filter,
		author$project$Main$hasACount(3),
		counts);
	var hasTwos = A2(
		elm$core$Dict$filter,
		author$project$Main$hasACount(2),
		counts);
	return {
		hasThree: elm$core$Dict$size(hasThrees) > 0,
		hasTwo: elm$core$Dict$size(hasTwos) > 0
	};
};
...
var author$project$Main$getChecksumValues = function (words) {
	var charList = A2(elm$core$List$map, elm$core$String$toList, words);
	var statusList = A2(elm$core$List$map, author$project$Main$checkWordStatus, charList);
	var sumOfThrees = A3(
		elm$core$List$foldl,
		F2(
			function (stat, total) {
				return total + (stat.hasThree ? 1 : 0);
			}),
		0,
		statusList);
	var sumOfTwos = A3(
...
var author$project$Main$sortString = function (inputString) {
	return elm$core$String$concat(
		A2(
			elm$core$List$map,
			elm$core$String$fromChar,
			elm$core$List$sort(
				elm$core$String$toList(inputString))));
};
...
var author$project$Main$init = function () {
	var counts = {threes: 0, twos: 0};
	var codes = elm$core$String$lines(author$project$Main$input);
	var sortedCodes = A2(elm$core$List$map, author$project$Main$sortString, codes);
	var newModel = author$project$Main$getChecksumValues(sortedCodes);
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text(
						'Checksum is ' + elm$core$String$fromInt(model.twos * model.threes))
					]))
			]));
};
