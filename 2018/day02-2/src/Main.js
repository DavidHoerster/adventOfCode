...
var author$project$Main$innerComparerHelper = F3(
	function (tupList, matches, countOfDiffs) {
		innerComparerHelper:
		while (true) {
			if (tupList.b) {
				var x = tupList.a;
				...
				if (_Utils_eq(f, s)) {
					var $temp$tupList = xs,
						$temp$matches = A2(elm$core$List$cons, f, matches),
						$temp$countOfDiffs = countOfDiffs;
					tupList = $temp$tupList;
					matches = $temp$matches;
					countOfDiffs = $temp$countOfDiffs;
					continue innerComparerHelper;
				} else {
					var $temp$tupList = xs,
						$temp$matches = matches,
						$temp$countOfDiffs = countOfDiffs + 1;
					tupList = $temp$tupList;
					matches = $temp$matches;
					countOfDiffs = $temp$countOfDiffs;
					continue innerComparerHelper;
...
var author$project$Main$innerComparer = F4(
	function (x1, x2, xs, curr) {
		innerComparer:
		while (true) {
			var tupList = A2(
				elm_community$list_extra$List$Extra$zip,
				elm$core$String$toList(x1),
				elm$core$String$toList(x2));
			var match = A3(author$project$Main$innerComparerHelper, tupList, _List_Nil, 0);
			if (match.a.b && (match.b === 1)) {
				var _n1 = match.a;
				var x = _n1.a;
				var xs1 = _n1.b;
				return A2(
...
var author$project$Main$processListHelper = F2(
	function (x, xs) {
		processListHelper:
		while (true) {
			var result = function () {
				if (xs.b) {
					var x1 = xs.a;
					var xs1 = xs.b;
					return A4(author$project$Main$innerComparer, x, x1, xs1, '');
				} else {
					return '';
				}
			}();
			if (elm$core$String$isEmpty(result)) {
				var t1 = function () {
					var _n1 = elm$core$List$tail(xs);
					if (_n1.$ === 'Just') {
						var aTail = _n1.a;
						return aTail;
					} else {
				...
				x = $temp$x;
				xs = $temp$xs;
				continue processListHelper;
			} else {
				return result;
			}
		}
	});
var author$project$Main$processList = function (list) {
	if (list.b) {
		var x = list.a;
		var xs = list.b;
		return A2(author$project$Main$processListHelper, x, xs);
	} else {
		return 'BLAH';
	}
};
...
var author$project$Main$init = function () {
	var result = author$project$Main$processList(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('Your code is :: ' + model)
					]))
			]));
};
